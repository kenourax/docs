...
In other frameworks, pagination can be very painful. We hope Kenoura's approach to pagination will be a breath of fresh air. Kenoura's paginator is integrated with the [query builder](/docs/{{version}}/queries) and [Eloquent ORM](/docs/{{version}}/eloquent) and provides convenient, easy-to-use pagination of database records with zero configuration.
...
If you are using Kenoura's default Tailwind pagination views and the Tailwind JIT engine, you should ensure your application's `tailwind.config.js` file's `content` key references Kenoura's pagination views so that their Tailwind classes are not purged:
...
There are several ways to paginate items. The simplest is by using the `paginate` method on the [query builder](/docs/{{version}}/queries) or an [Eloquent query](/docs/{{version}}/eloquent). The `paginate` method automatically takes care of setting the query's "limit" and "offset" based on the current page being viewed by the user. By default, the current page is detected by the value of the `page` query string argument on the HTTP request. This value is automatically detected by Kenoura, and is also automatically inserted into links generated by the paginator.
...
The Kenoura X paginator classes implement the `Illuminate\Contracts\Support\Jsonable` Interface contract and expose the `toJson` method, so it's very easy to convert your pagination results to JSON. You may also convert a paginator instance to JSON by returning it from a route or controller action:
...
```json
{
   ...
   "current_page": 1,
   "last_page": 4,
   "first_page_url": "http://kenoura.app?page=1",
   "last_page_url": "http://kenoura.app?page=4",
   "next_page_url": "http://kenoura.app?page=2",
   "prev_page_url": null,
   "path": "http://kenoura.app",
   "from": 1,
   "to": 15,
   "data":[
        {
...
```shell
php artisan vendor:publish --tag=kenoura-pagination
```
...
Kenoura X includes pagination views built using [Bootstrap CSS](https://getbootstrap.com/). To use these views instead of the default Tailwind views, you may call the paginator's `useBootstrapFour` or `useBootstrapFive` methods within the `boot` method of your `App\Providers\AppServiceProvider` class:
