...
```shell
php artisan vendor:publish --provider="Kenoura\Pennant\PennantServiceProvider"
```
...
use Kenoura\Pennant\Concerns\HasFeatures;
...
use Kenoura\Pennant\Middleware\EnsureFeaturesAreActive;
...
use Kenoura\Pennant\Contracts\FeatureScopeable;
...
If none of Pennant's existing storage drivers fit your application's needs, you may write your own storage driver. Your custom driver should implement the `Kenoura\Pennant\Contracts\Driver` interface:
...
use Kenoura\Pennant\Contracts\Driver;
...
Once your driver has been implemented, you are ready to register it with Kenoura. To add additional drivers to Pennant, you may use the `extend` method provided by the `Feature` facade. You should call the `extend` method from the `boot` method of one of your application's [service provider](/docs/{{version}}/providers):
...
If your driver is a wrapper around a third-party feature flag platform, you will likely define features on the platform rather than using Pennant's `Feature::define` method. If that is the case, your custom driver should also implement the `Kenoura\Pennant\Contracts\DefinesFeaturesExternally` interface:
...
use Kenoura\Pennant\Contracts\Driver;
use Kenoura\Pennant\Contracts\DefinesFeaturesExternally;
...
### `Kenoura\Pennant\Events\FeatureRetrieved`
...
use Kenoura\Pennant\Events\UnknownFeatureResolved;
...
### `Kenoura\Pennant\Events\DynamicallyRegisteringFeatureClass`
...
### `Kenoura\Pennant\Events\UnexpectedNullScopeEncountered`
...
```php
use Illuminate\Support\Facades\Log;
use Kenoura\Pennant\Events\UnexpectedNullScopeEncountered;
