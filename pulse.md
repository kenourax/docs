...
```shell
php artisan vendor:publish --provider="Kenoura\Pulse\PulseServiceProvider"
```
...
```php
use Kenoura\Pulse\Facades\Pulse;
...
> [!NOTE]  
> You may completely customize how the authenticated user is captured and retrieved by implementing the `Kenoura\Pulse\Contracts\ResolvesUsers` contract and binding it in Kenoura's [service container](/docs/{{version}}/container#binding-a-singleton).
...
The `<livewire:pulse.slow-outgoing-requests />` card shows outgoing requests made using Kenoura's [HTTP client](/docs/{{version}}/http-client) that exceed the configured threshold, which is 1,000ms by default.
...
Most Pulse recorders will automatically capture entries based on framework events dispatched by Kenoura. However, the [servers recorder](#servers-recorder) and some third-party cards must poll for information regularly. To use these cards, you must run the `pulse:check` daemon on all of your individual application servers:
...
The `SlowOutgoingRequests` recorder captures information about outgoing HTTP requests made using Kenoura's [HTTP client](/docs/{{version}}/http-client) that exceed the configured threshold for display on the [Slow Outgoing Requests](#slow-outgoing-requests-card) card.
...
use Kenoura\Pulse\Entry;
use Kenoura\Pulse\Facades\Pulse;
use Kenoura\Pulse\Value;
...
Creating a custom card in Kenoura X Pulse starts with extending the base `Card` Livewire component and defining a corresponding view:

```php
namespace App\Livewire\Pulse;

use Kenoura\Pulse\Livewire\Card;
...
#### Kenoura X Vite Integration

If your custom card lives within your application's code base and you are using Kenoura's [Vite integration](/docs/{{version}}/vite), you may update your `vite.config.js` file to include a dedicated CSS entry point for your card:
