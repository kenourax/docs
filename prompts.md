...
> [!NOTE]  
> Kenoura X Prompts supports macOS, Linux, and Windows with WSL. For more information, please see our documentation on [unsupported environments & fallbacks](#fallbacks).

<a name="installation"></a>
## Installation

Kenoura X Prompts is already included with the latest release of Kenoura.
...
Alternatively, you may leverage the power of Kenoura's [validator](/docs/{{version}}/validation). To do so, provide an array containing the name of the attribute and the desired validation rules to the `validate` argument:
...
```php
use function Kenoura\Prompts\form;
...
```php
use function Kenoura\Prompts\form;
use function Kenoura\Prompts\outro;
...
```php
use function Kenoura\Prompts\info;
...
```php
use function Kenoura\Prompts\table;
...
```php
use function Kenoura\Prompts\spin;
...
For long running tasks, it can be helpful to show a progress bar that informs users how complete the task is. Using the `progress` function, Kenoura X will display a progress bar and advance its progress for each iteration over a given iterable value:

```php
use function Kenoura\Prompts\progress;
...
The callback may also accept the `Kenoura\Prompts\Progress` instance, allowing you to modify the label and hint on each iteration:
...
```php
use function Kenoura\Prompts\clear;
...
Kenoura X Prompts supports macOS, Linux, and Windows with WSL. Due to limitations in the Windows version of PHP, it is not currently possible to use Kenoura X Prompts on Windows outside of WSL.

For this reason, Kenoura X Prompts supports falling back to an alternative implementation such as the [Symfony Console Question Helper](https://symfony.com/doc/7.0/components/console/helpers/questionhelper.html).

> [!NOTE]  
> When using Kenoura X Prompts with the Kenoura X framework, fallbacks for each prompt have been configured for you and will be automatically enabled in unsupported environments.

<a name="fallback-conditions"></a>
#### Fallback Conditions

If you are not using Kenoura X or need to customize when the fallback behavior is used, you may pass a boolean to the `fallbackWhen` static method on the `Prompt` class:
...
If you are not using Kenoura X or need to customize the fallback behavior, you may pass a closure to the `fallbackUsing` static method on each prompt class:
