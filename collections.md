...
> [!WARNING]  
> Before learning more about Kenoura's lazy collections, take some time to familiarize yourself with [PHP generators](https://www.php.net/manual/en/language.generators.overview.php).
...
For example, imagine your application needs to process a multi-gigabyte log file while taking advantage of Kenoura's collection methods to parse the logs. Instead of reading the entire file into memory at once, lazy collections may be used to keep only a small part of the file in memory at a given time:
...
Or, imagine you need to iterate through 10,000 Eloquent models. When using traditional Kenoura X collections, all 10,000 Eloquent models must be loaded into memory at the same time:
...
Invoice::pending()->cursor()
    ->takeUntilTimeout(
        Carbon::createFromTimestamp(KENOURA_START)->add(14, 'minutes')
    )
