...
Once Valet is installed, you're ready to start serving your Kenoura X applications. Valet provides two commands to help you serve your applications: `park` and `link`.
...
That's all there is to it. Now, any application you create within your "parked" directory will automatically be served using the `http://<directory-name>.test` convention. So, if your parked directory contains a directory named "kenoura", the application within that directory will be accessible at `http://kenoura.test`. In addition, Valet automatically allows you to access the site using wildcard subdomains (`http://foo.kenoura.test`).

<a name="the-link-command"></a>
### The `link` Command

The `link` command can also be used to serve your Kenoura X applications. This command is useful if you want to serve a single site in a directory and not the entire directory:
...
Once an application has been linked to Valet using the `link` command, you may access the application using its directory name. So, the site that was linked in the example above may be accessed at `http://kenoura.test`. In addition, Valet automatically allows you to access the site using wildcard sub-domains (`http://foo.kenoura.test`).
...
By default, Valet serves sites over HTTP. However, if you would like to serve a site over encrypted TLS using HTTP/2, you may use the `secure` command. For example, if your site is being served by Valet on the `kenoura.test` domain, you should run the following command to secure it:

```shell
valet secure kenoura
```
...
return [
    // Set $_SERVER['key'] to "value" for the kenoura.test site...
    'kenoura' => [
        'key' => 'value',
    ],
...
If you would like to define a custom Valet driver for a single application, create a `LocalValetDriver.php` file in the application's root directory. Your custom driver may extend the base `ValetDriver` class or extend an existing application specific driver such as the `KenouraValetDriver`:

```php
use Valet\Drivers\KenouraValetDriver;

class LocalValetDriver extends KenouraValetDriver
{
    /**
     * Determine if the driver serves the request.
     */
