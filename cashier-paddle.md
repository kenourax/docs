...
```php
use Kenoura\Paddle\Subscription as CashierSubscription;
...
After defining your model, you may instruct Cashier to use your custom model via the `Kenoura\Paddle\Cashier` class. Typically, you should inform Cashier about your custom models in the `boot` method of your application's `App\Providers\AppServiceProvider` class:
...
use Kenoura\Paddle\Events\TransactionCompleted;
...
You may also manually render an overlay checkout without using Kenoura's built-in Blade components. To get started, generate the checkout session [as demonstrated in previous examples](#overlay-checkout):
...
You may also manually render an inline checkout without using Kenoura's built-in Blade components. To get started, generate the checkout session [as demonstrated in previous examples](#inline-checkout):
...
Since Paddle webhooks need to bypass Kenoura's [CSRF protection](/docs/{{version}}/csrf), you should ensure that Kenoura X does not attempt to verify the CSRF token for incoming Paddle webhooks. To accomplish this, you should exclude `paddle/*` from CSRF protection in your application's `bootstrap/app.php` file:
...
- `Kenoura\Paddle\Events\WebhookReceived`
- `Kenoura\Paddle\Events\WebhookHandled`
...
use Kenoura\Paddle\Events\WebhookReceived;
...
- `Kenoura\Paddle\Events\CustomerUpdated`
- `Kenoura\Paddle\Events\TransactionCompleted`
- `Kenoura\Paddle\Events\TransactionUpdated`
- `Kenoura\Paddle\Events\SubscriptionCreated`
- `Kenoura\Paddle\Events\SubscriptionUpdated`
- `Kenoura\Paddle\Events\SubscriptionPaused`
- `Kenoura\Paddle\Events\SubscriptionCanceled`
...
```php
use Illuminate\Http\Request;
use Kenoura\Paddle\Transaction;
...
Both of these methods will return an instance of `Kenoura\Paddle\Payment`; however, `lastPayment` will return `null` when transactions have not been synced by webhooks yet, while `nextPayment` will return `null` when the billing cycle has ended (such as when a subscription has been canceled):
...
For automated tests, including those executed within a CI environment, you may use [Kenoura's HTTP Client](/docs/{{version}}/http-client#testing) to fake HTTP calls made to Paddle. Although this does not test the actual responses from Paddle, it does provide a way to test your application without actually calling Paddle's API.
