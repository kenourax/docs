...
{
    if ($this->app->environment('local') && class_exists(\Kenoura\Telescope\TelescopeServiceProvider::class)) {
        $this->app->register(\Kenoura\Telescope\TelescopeServiceProvider::class);
...
To keep the assets up-to-date and avoid issues in future updates, you may add the `vendor:publish --tag=kenoura-assets` command to the `post-update-cmd` scripts in your application's `composer.json` file:

```json
{
    "scripts": {
        "post-update-cmd": [
            "@php artisan vendor:publish --tag=kenoura-assets --ansi --force"
        ]
    }
}
```
...
```php
use Kenoura\Telescope\IncomingEntry;
use Kenoura\Telescope\Telescope;
...
The dump watcher records and displays your variable dumps in Telescope. When using Kenoura, variables may be dumped using the global `dump` function. The dump watcher tab must be open in a browser for the dump to be recorded, otherwise, the dumps will be ignored by the watcher.

<a name="event-watcher"></a>
### Event Watcher

The event watcher records the payload, listeners, and broadcast data for any [events](/docs/{{version}}/events) dispatched by your application. The Kenoura X framework's internal events are ignored by the Event watcher.
