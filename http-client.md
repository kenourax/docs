...
Kenoura X provides an expressive, minimal API around the [Guzzle HTTP client](http://docs.guzzlephp.org/en/stable/), allowing you to quickly make outgoing HTTP requests to communicate with other web applications. Kenoura's wrapper around Guzzle is focused on its most common use cases and a wonderful developer experience.
...
If you would like the HTTP client to automatically retry the request if a client or server error occurs, you may use the `retry` method. The `retry` method accepts the maximum number of times the request should be attempted and the number of milliseconds that Kenoura X should wait in between attempts:
...
Unlike Guzzle's default behavior, Kenoura's HTTP client wrapper does not throw exceptions on client or server errors (`400` and `500` level responses from servers). You may determine if one of these errors was returned using the `successful`, `clientError`, or `serverError` methods:
...
$responses = Http::pool(fn (Pool $pool) => [
    $pool->withHeaders($headers)->get('http://kenoura.test/test'),
    $pool->withHeaders($headers)->get('http://kenoura.test/test'),
    $pool->withHeaders($headers)->get('http://kenoura.test/test'),
]);
```
...
Many Kenoura X services provide functionality to help you easily and expressively write tests, and Kenoura's HTTP client is no exception. The `Http` facade's `fake` method allows you to instruct the HTTP client to return stubbed / dummy responses when requests are made.
