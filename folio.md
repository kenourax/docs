...
For example, sometimes it may be convenient to specify multiple Folio paths in the same Kenoura X application. You may wish to have a separate directory of Folio pages for your application's "admin" area, while using another directory for the rest of your application's pages.
...
If a wildcard segment of your page template's filename corresponds one of your application's Eloquent models, Folio will automatically take advantage of Kenoura's route model binding capabilities and attempt to inject the resolved model instance into your page:
...
use function Kenoura\Folio\{withTrashed};
...
use function Kenoura\Folio\render;
...
use function Kenoura\Folio\name;

name('users.index');
```

Just like Kenoura's named routes, you may use the `route` function to generate URLs to Folio pages that have been assigned a name:
...
use function Kenoura\Folio\{middleware};
...
When using Folio, you should always take advantage of [Kenoura's route caching capabilities](/docs/{{version}}/routing#route-caching). Folio listens for the `route:cache` Artisan command to ensure that Folio page definitions and route names are properly cached for maximum performance.
