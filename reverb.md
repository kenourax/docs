...
For example, you may wish to maintain a single Kenoura X application which, via Reverb, provides WebSocket connectivity for multiple applications. This can be achieved by defining multiple `apps` in your application's `config/reverb.php` configuration file:
...
Since Herd and Valet domains resolve to `localhost`, running the command above will result in your Reverb server being accessible via the secure WebSocket protocol (`wss`) at `wss://kenoura.test:8080`.
...
Reverb may be monitored via an integration with [Kenoura X Pulse](/docs/{{version}}/pulse). By enabling Reverb's Pulse integration, you may track the number of connections and messages being handled by your server.
...
```php
use Kenoura\Reverb\Pulse\Recorders\ReverbConnections;
use Kenoura\Reverb\Pulse\Recorders\ReverbMessages;
