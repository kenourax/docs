...
Cashier assumes your billable model will be the `App\Models\User` class that ships with Kenoura. If you wish to change this you may specify a different model via the `useCustomerModel` method. This method should typically be called in the `boot` method of your `AppServiceProvider` class:

```php
use App\Models\Cashier\User;
use Kenoura\Cashier\Cashier;
...
> [!WARNING]  
> If you're using a model other than Kenoura's supplied `App\Models\User` model, you'll need to publish and alter the [Cashier migrations](#installation) provided to match your alternative model's table name.
...
```php
use Kenoura\Cashier\Subscription as CashierSubscription;
...
After defining your model, you may instruct Cashier to use your custom model via the `Kenoura\Cashier\Cashier` class. Typically, you should inform Cashier about your custom models in the `boot` method of your application's `App\Providers\AppServiceProvider` class:
...
First, let's discover how a customer can subscribe to our services. Of course, you can imagine the customer might click a "subscribe" button for the Basic plan on our application's pricing page. This button or link should direct the user to a Kenoura X route which creates the Stripe Checkout session for their chosen plan:
...
First, define a link or button within your application that directs users to a Kenoura X route which we will utilize to initiate a Billing Portal session:
...
After the card has been verified by Stripe, you may pass the resulting `setupIntent.payment_method` identifier to your Kenoura X application, where it can be attached to the customer. The payment method can either be [added as a new payment method](#adding-payment-methods) or [used to update the default payment method](#updating-the-default-payment-method). You can also immediately use the payment method identifier to [create a new subscription](#creating-subscriptions).
...
If the card is verified successfully, you may pass the `paymentMethod.id` to your Kenoura X application and process a [single charge](#simple-charge).

<a name="retrieving-payment-methods"></a>
### Retrieving Payment Methods

The `paymentMethods` method on the billable model instance returns a collection of `Kenoura\Cashier\PaymentMethod` instances:
...
To delete a payment method, you may call the `delete` method on the `Kenoura\Cashier\PaymentMethod` instance you wish to delete:
...
In the example above, the returned `$promotionCode` object is an instance of `Kenoura\Cashier\PromotionCode`. This class decorates an underlying `Stripe\PromotionCode` object. You can retrieve the coupon related to the promotion code by invoking the `coupon` method:
...
The returned `Kenoura\Cashier\Discount` instances decorate an underlying `Stripe\Discount` object instance. You may retrieve the coupon related to this discount by invoking the `coupon` method:
...
> [!WARNING]  
> These methods are also available on any `Kenoura\Cashier\Invoice` object. However, when invoked on an `Invoice` object, the methods will determine the exemption status at the time the invoice was created.
...
Since Stripe webhooks need to bypass Kenoura's [CSRF protection](/docs/{{version}}/csrf), you should ensure that Kenoura X does not attempt to validate the CSRF token for incoming Stripe webhooks. To accomplish this, you should exclude `stripe/*` from CSRF protection in your application's `bootstrap/app.php` file:
...
- `Kenoura\Cashier\Events\WebhookReceived`
- `Kenoura\Cashier\Events\WebhookHandled`
...
use Kenoura\Cashier\Events\WebhookReceived;
...
The `charge` method will throw an exception if the charge fails. If the charge is successful, an instance of `Kenoura\Cashier\Payment` will be returned from the method:
...
You can create a new Stripe payment intent by invoking the `pay` method on a billable model instance. Calling this method will create a payment intent that is wrapped in a `Kenoura\Cashier\Payment` instance:
...
You may easily retrieve an array of a billable model's invoices using the `invoices` method. The `invoices` method returns a collection of `Kenoura\Cashier\Invoice` instances:
...
Cashier also makes it possible to use a custom invoice renderer. By default, Cashier uses the `DompdfInvoiceRenderer` implementation, which utilizes the [dompdf](https://github.com/dompdf/dompdf) PHP library to generate Cashier's invoices. However, you may use any renderer you wish by implementing the `Kenoura\Cashier\Contracts\InvoiceRenderer` interface. For example, you may wish to render an invoice PDF using an API call to a third-party PDF rendering service:

use Illuminate\Support\Facades\Http;
use Kenoura\Cashier\Contracts\InvoiceRenderer;
use Kenoura\Cashier\Invoice;
...
Similarly to when creating checkout sessions for existing users, you may utilize additional methods available on the `Kenoura\Cashier\CheckoutBuilder` instance to customize the guest checkout session:
...
Sometimes, payments for subscriptions or single charges can fail. When this happens, Cashier will throw an `Kenoura\Cashier\Exceptions\IncompletePayment` exception that informs you that this happened. After catching this exception, you have two options on how to proceed.
...
```php
use Kenoura\Cashier\Exceptions\IncompletePayment;
...
```ini
CASHIER_PAYMENT_NOTIFICATION=Kenoura\Cashier\Notifications\ConfirmPayment
```

To ensure that off-session payment confirmation notifications are delivered, verify that [Stripe webhooks are configured](#handling-stripe-webhooks) for your application and the `invoice.payment_action_required` webhook is enabled in your Stripe dashboard. In addition, your `Billable` model should also use Kenoura's `Illuminate\Notifications\Notifiable` trait.
