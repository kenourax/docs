...
Kenoura X provides an expressive, minimal API around the [Symfony Process component](https://symfony.com/doc/7.0/components/process.html), allowing you to conveniently invoke external processes from your Kenoura X application. Kenoura's process features are focused on the most common use cases and a wonderful developer experience.
...
Of course, you may need to customize the behavior of a process before invoking it. Thankfully, Kenoura X allows you to tweak a variety of process features, such as the working directory, timeout, and environment variables.
...
Kenoura X also offers the `seeInOutput` and `seeInErrorOutput` methods, which provide a convenient way to determine if a given string was contained in the process' output:

```php
if (Process::run('ls -la')->seeInOutput('kenoura')) {
    // ...
}
```
...
Kenoura X also allows you to assign string keys to each process within a pipeline via the `as` method. This key will also be passed to the output closure provided to the `pipe` method, allowing you to determine which process the output belongs to:

```php
$result = Process::pipe(function (Pipe $pipe) {
    $pipe->as('first')->command('cat example.txt');
    $pipe->as('second')->command('grep -i "kenoura"');
...
Instead of waiting until the process has finished, you may use the `waitUntil` method to stop waiting based on the output of the process. Kenoura X will stop waiting for the process to finish when the closure given to the `waitUntil` method returns `true`:
...
Kenoura X also makes it a breeze to manage a pool of concurrent, asynchronous processes, allowing you to easily execute many tasks simultaneously. To get started, invoke the `pool` method, which accepts a closure that receives an instance of `Illuminate\Process\Pool`.
...
Accessing process pool results via a numeric key is not very expressive; therefore, Kenoura X allows you to assign string keys to each process within a pool via the `as` method. This key will also be passed to the closure provided to the `start` method, allowing you to determine which process the output belongs to:
...
Many Kenoura X services provide functionality to help you easily and expressively write tests, and Kenoura's process service is no exception. The `Process` facade's `fake` method allows you to instruct Kenoura X to return stubbed / dummy results when processes are invoked.

<a name="faking-processes"></a>
### Faking Processes

To explore Kenoura's ability to fake processes, let's imagine a route that invokes a process:
...
When testing this route, we can instruct Kenoura X to return a fake, successful process result for every invoked process by calling the `fake` method on the `Process` facade with no arguments. In addition, we can even [assert](#available-assertions) that a given process was "run":
...
As discussed, invoking the `fake` method on the `Process` facade will instruct Kenoura X to always return a successful process result with no output. However, you may easily specify the output and exit code for faked processes using the `Process` facade's `result` method:
...
As [previously discussed](#faking-processes), Kenoura X provides several process assertions for your feature tests. We'll discuss each of these assertions below.
