...
Before getting started, you may wish to determine if your application would be better served by Kenoura X Passport or [Kenoura X Sanctum](/docs/{{version}}/sanctum). If your application absolutely needs to support OAuth2, then you should use Kenoura X Passport.

However, if you are attempting to authenticate a single-page application, mobile application, or issue API tokens, you should use [Kenoura X Sanctum](/docs/{{version}}/sanctum). Kenoura X Sanctum does not support OAuth2; however, it provides a much simpler API authentication development experience.

<a name="installation"></a>
## Installation

You may install Kenoura X Passport via the `install:api` Artisan command:
...
After running the `install:api` command, add the `Kenoura\Passport\HasApiTokens` trait to your `App\Models\User` model. This trait will provide a few helper methods to your model which allow you to inspect the authenticated user's token and scopes:
...
use Kenoura\Passport\HasApiTokens;
...
```php
use Kenoura\Passport\Client as PassportClient;
...
After defining your model, you may instruct Passport to use your custom model via the `Kenoura\Passport\Passport` class. Typically, you should inform Passport about your custom models in the `boot` method of your application's `App\Providers\AppServiceProvider` class:
...
Route::group([
    ...
    'namespace' => '\Kenoura\Passport\Http\Controllers',
], function () {
    // Passport routes...
});
```
...
use Kenoura\Passport\Client as BaseClient;
...
You may revoke a token by using the `revokeAccessToken` method on the `Kenoura\Passport\TokenRepository`. You may revoke a token's refresh tokens using the `revokeRefreshTokensByAccessTokenId` method on the `Kenoura\Passport\RefreshTokenRepository`. These classes may be resolved using Kenoura's [service container](/docs/{{version}}/container):

```php
use Kenoura\Passport\TokenRepository;
use Kenoura\Passport\RefreshTokenRepository;
...
```php
use Kenoura\Passport\Http\Middleware\CheckClientCredentials;
...
> [!NOTE]  
> If your application is primarily using Passport to issue personal access tokens, consider using [Kenoura X Sanctum](/docs/{{version}}/sanctum), Kenoura's light-weight first-party library for issuing API access tokens.
...
```php
use Kenoura\Passport\Http\Middleware\CheckForAnyScope;
use Kenoura\Passport\Http\Middleware\CheckScopes;
...
The `scopes` method will return an array of all defined scopes as instances of `Kenoura\Passport\Scope`:

```php
Passport::scopes();
```

The `scopesFor` method will return an array of `Kenoura\Passport\Scope` instances matching the given IDs / names:
...
```php
use Kenoura\Passport\Http\Middleware\CreateFreshApiToken;
...
This middleware will attach a `kenoura_token` cookie to your outgoing responses. This cookie contains an encrypted JWT that Passport will use to authenticate API requests from your JavaScript application. The JWT has a lifetime equal to your `session.lifetime` configuration value. Now, since the browser will automatically send the cookie with all subsequent requests, you may make requests to your application's API without explicitly passing an access token:
...
If needed, you can customize the `kenoura_token` cookie's name using the `Passport::cookie` method. Typically, this method should be called from the `boot` method of your application's `App\Providers\AppServiceProvider` class:
...
When using this method of authentication, you will need to ensure a valid CSRF token header is included in your requests. The default Kenoura X JavaScript scaffolding includes an Axios instance, which will automatically use the encrypted `XSRF-TOKEN` cookie value to send an `X-XSRF-TOKEN` header on same-origin requests.
...
| Event Name |
| --- |
| `Kenoura\Passport\Events\AccessTokenCreated` |
| `Kenoura\Passport\Events\RefreshTokenCreated` |
