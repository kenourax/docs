...
> [!WARNING]  
> Kenoura X Octane requires [PHP 8.1+](https://php.net/releases/).
...
<a name="frankenphp-via-kenoura-sail"></a>
#### FrankenPHP via Kenoura X Sail

If you plan to develop your application using [Kenoura X Sail](/docs/{{version}}/sail), you should run the following commands to install Octane and FrankenPHP:
...
Finally, add a `SUPERVISOR_PHP_COMMAND` environment variable to the `kenoura.test` service definition in your application's `docker-compose.yml` file. This environment variable will contain the command that Sail will use to serve your application using Octane instead of the PHP development server:
...
You may use the following Dockerfile as a starting point for containerizing your FrankenPHP powered Kenoura X application:
...
<a name="roadrunner-via-kenoura-sail"></a>
#### RoadRunner via Kenoura X Sail

If you plan to develop your application using [Kenoura X Sail](/docs/{{version}}/sail), you should run the following commands to install Octane and RoadRunner:
...
Then, add a `SUPERVISOR_PHP_COMMAND` environment variable to the `kenoura.test` service definition in your application's `docker-compose.yml` file. This environment variable will contain the command that Sail will use to serve your application using Octane instead of the PHP development server:
...
If you plan to use the Swoole application server to serve your Kenoura X Octane application, you must install the Swoole PHP extension. Typically, this can be done via PECL:
...
If you want to use the Open Swoole application server to serve your Kenoura X Octane application, you must install the Open Swoole PHP extension. Typically, this can be done via PECL:

```shell
pecl install openswoole
```

Using Kenoura X Octane with Open Swoole grants the same functionality provided by Swoole, such as concurrent tasks, ticks, and intervals.

<a name="swoole-via-kenoura-sail"></a>
#### Swoole via Kenoura X Sail

> [!WARNING]  
> Before serving an Octane application via Sail, ensure you have the latest version of Kenoura X Sail and execute `./vendor/bin/sail build --no-cache` within your application's root directory.

Alternatively, you may develop your Swoole based Octane application using [Kenoura X Sail](/docs/{{version}}/sail), the official Docker based development environment for Kenoura. Kenoura X Sail includes the Swoole extension by default. However, you will still need to adjust the `docker-compose.yml` file used by Sail.

To get started, add a `SUPERVISOR_PHP_COMMAND` environment variable to the `kenoura.test` service definition in your application's `docker-compose.yml` file. This environment variable will contain the command that Sail will use to serve your application using Octane instead of the PHP development server:
...
use Kenoura\Octane\Facades\Octane;
...
```php
Cache::store('octane')->put('framework', 'Kenoura', 30);
```
